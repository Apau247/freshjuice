...
<div class="card border-0 shadow-sm">
    <div class="card-body">
        <form method="POST" action="<?= $isEdit ? '?route=power/usage/edit&id=' . urlencode($record['PowerUsageID']) : '?route=power/usage/create' ?>" class="row g-3">
            <?= csrfField() ?>
            <div class="col-md-4">
            ...
            <div class="col-md-4">
                ...
                <select name="source" class="form-select" required>
                    ...
                    <?php $src = $record['Source'] ?? ''; ?>
                    <option value="Grid" <?= $src === 'Grid' ? 'selected' : '' ?>>Grid</option>
                    <option value="Generator" <?= $src === 'Generator' ? 'selected' : '' ?>>Generator</option>
                    <option value="Solar" <?= $src === 'Solar' ? 'selected' : '' ?>>Solar</option>
                </select>
            </div>
            <div class="col-md-4">
